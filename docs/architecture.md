...
> **Status:** This document describes the implemented RecallOps MVP architecture and the boundaries that remain intentionally out of scope.
...
- Amazon Bedrock provides the chat model and embeddings.
- The exact chat model is configured by environment variable.
- Titan Text Embeddings V2 is the embedding model.
- AWS ECS Express Mode hosts the public demo service from the ECR image.
...
- Future application layers would need to control agent steps, tool permissions, persistence, and observability before RecallOps can safely execute external actions.
